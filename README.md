...
Different kinetics are used for proximal and distal dendritic KA channels, with increasing density with distance from the soma.
The kinetics for the Na conductance includes a gate variable for the slow inactivation.

To compile the mod files on a unix system use the command `nrnivmodl`.
Under Windows use the "mknrndll DOS box" from the NEURON program menu and follow on-screen instructions.
...
will generate the simulations shown in Fig.1A and Fig.1C of the paper, respectively.
